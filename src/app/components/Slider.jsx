...
const Slider = () => {
  ...
  useEffect(() => {
    const getItemDimensions = () => {
      if (window.innerWidth < 640) return { itemWidth: 140, gap: 10, visibleItems: 1.8 }
      if (window.innerWidth < 768) return { itemWidth: 170, gap: 12, visibleItems: 2.5 }
      if (window.innerWidth < 1024) return { itemWidth: 200, gap: 14, visibleItems: 3.5 }
    ...
    handleResize() // ustaw początkowe wymiary po stronie klienta
  ...
  const handleScrollLeft = () => {
    setScrollPosition(prev => Math.max(prev - itemWithGap * 2, 0))
  }

  const handleScrollRight = () => {
    setScrollPosition(prev => Math.min(prev + itemWithGap * 2, maxScroll))
  }
